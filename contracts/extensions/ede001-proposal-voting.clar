...
(define-public (is-dao-or-extension)
	(ok (asserts! (or (is-eq tx-sender .bitcoin-l2-dao) (contract-call? .bitcoin-l2-dao is-extension contract-caller)) err-unauthorised))
)
...
(define-public (add-proposal (proposal <proposal-trait>) (data {start-block-height: uint, end-block-height: uint, proposer: principal}))
	(begin
		(try! (is-dao-or-extension))
		(asserts! (is-none (contract-call? .bitcoin-l2-dao executed-at proposal)) err-proposal-already-executed)
...
(define-public (conclude (proposal <proposal-trait>))
	(let
		...
		(and passed (try! (contract-call? .bitcoin-l2-dao execute proposal tx-sender)))
		(ok passed)
	)
)
