;; Title: BDE005 Dev Fund
;; Author: Marvin Janssen
;; Depends-On: BDP000
;; Synopsis:
;; A simple pre-seeded dev fund that can pay out developers on a monthly basis.
;; Description:
;; Initialised by BDP001 Dev Fund. Developers can be awarded a monthly allowance
...
(define-public (transfer (amount uint) (recipient principal) (memo (optional (buff 34))))
	(begin
		(try! (is-dao-or-extension))
		(as-contract (contract-call? .bde000-governance-token transfer amount tx-sender recipient memo))
	)
)
...
(define-public (claim (memo (optional (buff 34))))
	(let
		...
		(as-contract (contract-call? .bde000-governance-token transfer (* (- max-claims claim-count) (get allowance entry)) tx-sender developer memo))
	)
)
