...
import { ExecutorDaoClient } from "./bitcoin-l2-dao-client.ts";
...
export class Utils {
  ...
  setup = (chain: Chain, accounts: Map<string, Account>): {
    ...
    const contractEXD = accounts.get("deployer")!.address + '.bitcoin-l2-dao';
    ...
    const exeDaoClient = new ExecutorDaoClient(chain, deployer, 'bitcoin-l2-dao');
