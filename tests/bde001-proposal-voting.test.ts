...
import { constructDao, coreProposals, deployer, errors, proposalVoting } from "./helpers";
...
describe("bde001-proposal-voting", () => {
  ...
  test("proposal-voting - cannot conclude before end height", async () => {
    ...
    expect(response1.value).toBe(errors.proposalVoting.errEndBurnHeightNotReached)
  ...
  test("proposal-voting - wins to custom majority", async () => {
    ...
    // pass proposal to change sunset
    const proposal1 = simnet.deployer + '.' + 'bdp000-core-team-sunset-height'
    let response = txOk(coreProposals.corePropose(proposal1, (simnet.blockHeight+5), 100, 8000), alice);
    expect(response.value).toBe(true)
  ...
  test("proposal-voting - alice can vote with entire balance below voting cap", async () => {
    ...
    simnet.transferSTX(diff, accounts.wallet_2.address, accounts.wallet_1.address)
    simnet.transferSTX(1n, accounts.wallet_2.address, accounts.wallet_1.address)
